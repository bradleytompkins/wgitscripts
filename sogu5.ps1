...
# Shannon entropy (bits/byte, 0-8) over up to the first 256 KB of a file.
# Encrypted/compressed data (e.g. a SOGU payload) runs ~7.9+; text/config/most
# benign .dat files run much lower. Used to separate a real payload from noise.
function FileEntropy([string]$path) {
    try {
        ...
        $ent = 0.0
        foreach ($c in $freq) {
            if ($c -gt 0) { $p = $c / $read; $ent -= $p * [Math]::Log($p, 2) }
        }
...
# Side-load pass: one directory at a time, using files captured above.
# Two shapes:
#   (a) exe + dll + dat  -- the classic side-load triad.
#   (b) dll + dat, NO exe -- the loader DLL is launched by a service/rundll32,
#       not a co-located exe, so there is no exe to find. This shape was being
#       missed entirely. We require the DLL unsigned and the .dat non-benign,
#       and use entropy to tell an encrypted SOGU payload from an ordinary .dat.
# Signature checks (IsSigned) only fire here, on the few candidate directories.
foreach ($d in $dirFiles.Keys) {
    ...
    elseif ($dlls.Count -and $dats.Count) {
        # (b) loader DLL + payload .dat with no co-located exe.
        $unsignedDll = @($dlls | Where-Object { -not (IsSigned $_.FullName) })
        if ($named -ge 1) {
            Hit 'HIGH' "Loader DLL + .dat with known PlugX filename (no exe): $d"
        }
        elseif ($unsignedDll.Count) {
            $hotDat = @($dats | Where-Object { (FileEntropy $_.FullName) -ge 7.2 })
            if ($hotDat.Count) {
                $ent = (FileEntropy $hotDat[0].FullName)
                Hit 'HIGH' "Unsigned DLL + high-entropy .dat payload (entropy $ent, no exe): $d"
            } else {
                Hit 'MED'  "Unsigned DLL + non-benign .dat, no exe (possible loader+payload): $d"
            }
        }
    }
}
...
# Resolve what a service actually runs/loads: a svchost ServiceDll (from the
# registry), a rundll32/regsvr32 DLL argument, or a plain ImagePath exe.
function Resolve-ServicePayload($svc) {
    $img = $svc.PathName; $out = @()
    if (-not $img) { return $out }
    if ($img -match '(?i)\\svchost\.exe') {
        try {
            $sd = (Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\$($svc.Name)\Parameters" `
                     -Name ServiceDll -EA Stop).ServiceDll
            if ($sd) { $out += [Environment]::ExpandEnvironmentVariables($sd) }
        } catch {}
    }
    elseif ($img -match '(?i)\\(rundll32|regsvr32)\.exe') {
        if ($img -match '(?i)([a-z]:\\[^,"]+\.dll)') { $out += $matches[1].Trim() }
    }
    elseif ($img -match '(?i)"?([a-z]:\\[^"]+\.exe)') { $out += $matches[1] }
    @($out | Where-Object { $_ } | Select-Object -Unique)
}

$systemDirRx   = '(?i)\\Windows\\(system32|syswow64|winsxs)\\'
$userWritableRx = '(?i)(\\ProgramData\\|\\Users\\Public\\|\\AppData\\|\\Temp\\|\\Windows\\Temp\\|^[a-z]:\\Users\\)'

Get-CimInstance Win32_Service -EA SilentlyContinue | ForEach-Object {
    $svc = $_; $img = $svc.PathName
    if (-not $img) { return }

    # Known IOC string anywhere in the ImagePath.
    if ($img -match '(?i)(RECYCLER|AvastSvc|wsc\.dll|c3lzLmluZm8)') {
        Hit 'HIGH' "Service IOC: $($svc.Name) -> $img"; return
    }

    foreach ($payload in (Resolve-ServicePayload $svc)) {
        $exists   = Test-Path -LiteralPath $payload -EA SilentlyContinue
        $inSystem = $payload -match $systemDirRx
        $inUser   = $payload -match $userWritableRx
        $signed   = $exists -and (IsSigned $payload)
        $isDll    = $payload -match '(?i)\.dll$'

        # A service DLL (esp. a svchost ServiceDll) loading from OUTSIDE the
        # Windows system dirs is the PlugX persistence shape. Legit third-party
        # service DLLs are normally signed and in Program Files; flag when the
        # DLL is user-writable or unsigned.
        if ($isDll -and -not $inSystem -and ($inUser -or -not $signed)) {
            $why = if ($img -match '(?i)svchost') { 'svchost ServiceDll outside System32' }
                   elseif ($img -match '(?i)rundll32|regsvr32') { 'service LOLBin-loads a DLL' }
                   else { 'service loads a non-system DLL' }
            Hit 'HIGH' "Suspicious service ($why): $($svc.Name) -> $payload [img: $img]"

            # Look for an encrypted payload sitting next to the loader DLL -- the
            # ".dll and .dat side by side" shape, even if outside the scan roots.
            $dir = Split-Path $payload -Parent
            Get-ChildItem -LiteralPath $dir -Filter *.dat -Force -EA SilentlyContinue | ForEach-Object {
                if ($benignDat -notcontains $_.Name -and (FileEntropy $_.FullName) -ge 7.2) {
                    Hit 'HIGH' "Encrypted payload beside service DLL (entropy $(FileEntropy $_.FullName)): $($_.FullName)"
                }
            }
            continue
        }

        # Plain exe ImagePath: unsigned binary run from a user-writable path.
        if ($exists -and $inUser -and -not $signed) {
            Hit 'HIGH' "Service runs unsigned binary from user-writable path: $($svc.Name) -> $payload"
        }
    }
}
